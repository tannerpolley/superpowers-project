...
Use as many native questions and options as the decision requires. Batch independent questions; ask dependent branches sequentially. Use more than three peer options or independent questions when that is the clearest honest menu. Put the recommended option first when codebase evidence supports one, but still ask unless the user explicitly authorized automatic recommended defaults.
