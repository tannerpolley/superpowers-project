...
- `Run Audit Again`: rerun `$superpowers-project:audit-project` after changes or new GitHub evidence.
...
- `Review First`: show the audit summary and rendered artifacts for user review, then return to `project_doctor_next_step`.
- `Gather More Evidence`: inspect the requested source, then return to `project_doctor_next_step`.
