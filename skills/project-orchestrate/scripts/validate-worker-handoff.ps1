...
try {
    ...
    $expectedEvidence = "project-orchestrate-issue-$($identity.issue_number)-$($identity.issue_slug)"
    ...
    if ([string]$handoff.topology_handoff.merge_owner -ne "project-merge") { throw "merge_owner must be project-merge" }
