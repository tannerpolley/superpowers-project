...
function New-QuickApplyFixture {
    $root = Join-Path ([IO.Path]::GetTempPath()) "write-plan-quick-apply-$([guid]::NewGuid().ToString('N'))"
...
foreach ($fixtureRoot in $script:quickApplyFixtureRoots) {
    ...
    if ($resolved.StartsWith([IO.Path]::GetTempPath(), [StringComparison]::OrdinalIgnoreCase) -and (Split-Path $resolved -Leaf).StartsWith("write-plan-quick-apply-", [StringComparison]::Ordinal)) {
