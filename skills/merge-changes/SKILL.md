...
When the linked issue mirror is a hierarchy leaf, closeout records read-only parent rollup evidence: parent issue URL, parent mirror, sibling child states, `subIssuesSummary` progress, and local mirror receipts. This evidence does not close a parent or wrapper issue. Parent or wrapper rollup closeout requires a separate native approval ledger plus proof that every required child is closed or explicitly skipped.
