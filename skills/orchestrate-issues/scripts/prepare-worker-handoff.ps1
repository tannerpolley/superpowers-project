...
try {
    ...
    $validator = Join-Path $root "skills/create-issues/scripts/validate-issue-mirror.ps1"
    if (-not (Test-Path -LiteralPath $validator -PathType Leaf)) {
        $validator = Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) "create-issues/scripts/validate-issue-mirror.ps1"
    }
