...
- GitHub Milestones carry roadmap and milestone tracking; parent issues group work inside a milestone but do not replace the milestone.
...
- Parent and plan-wrapper issue mirrors are rollup records with `Executable: false`.
...
- Leaf merge closeout records `hierarchy_rollup` parent progress from GitHub `subIssuesSummary`; parent or wrapper closeout remains native-approval gated.
- `align-project` reports parent/sub-issue drift and clean-title migration candidates before any GitHub mutation.
- `loop-controller` skips parent and plan-wrapper implementation candidates and reserves them for rollup, alignment, or tracker repair.
...
| `create-issues` | Use when a Superpowers Project spec, plan, PRD, or approved scope needs vertical-slice GitHub issues and synced issue mirrors. | `project_issue_execution_route`<br>`project_issue_hierarchy_children`<br>`project_issue_hierarchy_mode`<br>`project_issue_hierarchy_parent`<br>`project_issue_hierarchy_publish`<br>`project_issue_hierarchy_tracker_fields`<br>`project_issue_hierarchy_wrapper`<br>`project_issue_next_step`<br>`project_issue_orchestrate_route`<br>`project_issue_reiteration_route`<br>`project_issue_resolve_route`<br>`project_issue_review_repair_route` | `None` |
...
| `initiate-workflow` | Route Superpowers Project extension requests to project setup, brainstorming, audits, planning, issue creation, issue triage, alignment, or goal-backed resolution workflows. | `project_auto_mode_authorization`<br>`project_issue_resolution_route`<br>`project_workflow_mode` | `None` |
...
| `setup-project` | Create or maintain the Superpowers Project setup, milestone map, GitHub tracker configuration, GitHub Project board configuration, and roadmap artifacts under docs/superpowers. | `project_setup_board_approval`<br>`project_setup_next_step`<br>`project_setup_reiteration_group`<br>`project_setup_reiteration_route`<br>`project_setup_work_route` | `None` |
