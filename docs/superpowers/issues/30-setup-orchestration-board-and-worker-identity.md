...
**Source Plan:** docs/superpowers/plans/2026-06-03-project-setup-orchestration-plan.md
**Classification:** HITL
...
- [ ] `project-setup` can prepare GitHub Project board setup and records config only after native approval.
- [ ] `project-orchestrate` derives consistent worker thread title, branch name, worktree path, evidence path, and PR title hints from an issue mirror.
- [ ] Worker handoff ledgers carry enough evidence for reviewer and merge closeout.
- [ ] `project-resolve` remains the direct current-thread route and does not create worker threads.
...
- User approval before any real GitHub Project board mutation.
