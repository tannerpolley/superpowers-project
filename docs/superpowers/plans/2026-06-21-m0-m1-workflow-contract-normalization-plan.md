...
**Tech Stack:** Bash validators and fixtures, YAML route contract, Markdown skill/spec/plan docs, existing Superpowers Project scripts, GitHub issue mirrors, and the repo validation suite.
...
**Interface:** Bash validators read the YAML contract and Markdown/YAML surfaces, then produce pass/fail results used by `scripts/validate.sh`, issue acceptance, and merge closeout.
...
**Acceptance Proof:** `scripts/validate.sh`, focused new validators, `scripts/sync-live.sh --validate`, and the cleanup hook all pass after the issue-backed implementation slices merge.
...
**Files To Modify:** workflow `skills/*/SKILL.md`, `skills/*/agents/openai.yaml`, `scripts/validate.sh`, existing focused tests, `README.md`, `docs/superpowers/OUTCOME_WORKFLOW.md` generator and generated output, milestone pages, and issue mirrors created from this plan.
...
**Write Path:** Edits are made in this source repo only, then synced with `scripts/sync-live.sh --validate`.
**Integration Points:** `scripts/validate.sh`, `scripts/generate-outcome-workflow-summary.sh`, `scripts/test-native-continuation-loop.sh`, `scripts/test-skill-metadata-readability.sh`, loop-controller candidate selection, create-issues mirror validation, orchestrate worker handoff validation, and merge closeout proof.
...
- [ ] `scripts/validate.sh`, `scripts/sync-live.sh --validate`, and the repo cleanup hook pass before final Done.
...
- Create: `scripts/lib/workflow-contract.sh`
- Create: `scripts/validate-workflow-contract.sh`
- Create: `scripts/test-workflow-contract.sh`
- Modify: `scripts/validate.sh`
- Test: `scripts/test-workflow-contract.sh`

- [ ] **Step 1: Write failing contract tests**
  - Add fixtures inside `scripts/test-workflow-contract.sh` that create a temporary contract and temporary skill/metadata surfaces.
...
- [ ] **Step 3: Implement contract loading helpers**
  - Add `scripts/lib/workflow-contract.sh` with helpers that load YAML through Bash, normalize question IDs, and expose skill entries by name.
  - Use the repo's existing Python/PyYAML expectation only when Bash cannot parse YAML directly.
- [ ] **Step 4: Implement the validator**
  - Add `scripts/validate-workflow-contract.sh` to compare contract entries against `skills/*/SKILL.md`, `skills/*/agents/openai.yaml`, and generated workflow summary expectations.
...
- [ ] **Step 5: Wire validation**
  - Add `scripts/test-workflow-contract.sh` and `scripts/validate-workflow-contract.sh` to `scripts/validate.sh`.
  - Run: `./scripts/test-workflow-contract.sh`
  - Expected: passes.
- [ ] **Step 6: Commit**
...
- Modify: `scripts/test-skill-metadata-readability.sh`
- Create: `scripts/validate-skill-metadata-contract.sh`
- Create: `scripts/test-skill-metadata-contract.sh`
- Modify: `scripts/validate.sh`
- Test: `scripts/test-skill-metadata-readability.sh`
...
- [ ] **Step 4: Wire validation**
  - Add the new metadata contract test and validator to `scripts/validate.sh`.
  - Run: `./scripts/test-skill-metadata-contract.sh`
  - Expected: passes.
- [ ] **Step 5: Run readability checks**
  - Run: `./scripts/test-skill-metadata-readability.sh`
...
- Create: `scripts/validate-global-policy-deduplication.sh`
- Create: `scripts/test-global-policy-deduplication.sh`
...
- [ ] **Step 4: Update continuation tests**
  - Modify `scripts/test-native-continuation-loop.sh` so it validates central policy presence in `advanced-user-input` and route-specific references in other skills.
  - Keep checks for top-level `Yes`, `Revisit`, `Stop`, final `Done`, and nested terminal restrictions.
- [ ] **Step 5: Run focused tests**
  - Run: `./scripts/test-global-policy-deduplication.sh`
  - Run: `./scripts/test-native-continuation-loop.sh`
  - Expected: both pass.
- [ ] **Step 6: Commit**
...
- Create: `scripts/test-decision-ledger.sh`
- Modify: `scripts/validate.sh`
...
- [ ] **Step 4: Wire validation**
  - Add focused tests to `scripts/validate.sh`.
...
- Create: `scripts/validate-artifact-review-card.sh`
- Create: `scripts/test-artifact-review-card.sh`
- Modify: `scripts/validate.sh`
...
- [ ] **Step 4: Implement validator**
  - Create `scripts/validate-artifact-review-card.sh` to validate Markdown card fixtures.
...
- Create: `scripts/validate-active-backlog.sh`
- Create: `scripts/test-active-backlog.sh`
- Modify: `scripts/validate.sh`
- Test: `scripts/test-active-backlog.sh`
...
- [ ] **Step 1: Write active-backlog tests**
  ...
  - Run: `./scripts/test-active-backlog.sh`
...
- [ ] **Step 4: Implement validator**
  - Create `scripts/validate-active-backlog.sh` to reject missing source artifacts, unsupported route owners, and historical plan checkbox-only candidates.
- [ ] **Step 5: Run focused tests**
  - Run: `./scripts/test-active-backlog.sh`
...
**Files:**
- Modify: `.gitignore`
- Create: `scripts/validate-generated-state.sh`
- Create: `scripts/test-generated-state.sh`
- Modify: `scripts/validate-flat-artifact-roots.sh`
- Modify: `scripts/validate.sh`
- Test: `scripts/test-generated-state.sh`
...
- [ ] **Step 1: Write generated-state tests**
  ...
  - Run: `./scripts/test-generated-state.sh`
  - Expected: fails until validator exists.
...
- [ ] **Step 4: Wire validation**
  - Add the generated-state validator to `scripts/validate.sh`.
...
- Create: `scripts/validate-workflow-examples.sh`
- Create: `scripts/test-workflow-examples.sh`
...
- [ ] **Step 4: Wire validation**
  - Add example tests and validator to `scripts/validate.sh`.
...
- Create: `scripts/test-worker-packets.sh`
- Modify: `scripts/validate.sh`
- Test: `skills/orchestrate-issues/scripts/test-scenarios.sh`
- Test: `scripts/test-worker-packets.sh`
...
- [ ] **Step 1: Write packet tests**
  ...
  - Run: `./scripts/test-worker-packets.sh`
...
- [ ] **Step 4: Extend handoff validator**
  - Update `validate-worker-handoff.sh` to validate the fields used by the examples.
...
- [ ] **Step 3: Run version check**
  - Run: `./scripts/get-agent-plugin-version.sh -Banner -RequireCurrent`
...
- `./scripts/validate-plan-outcome-proof.sh -PlanPath ./docs/superpowers\plans\2026-06-21-m0-m1-workflow-contract-normalization-plan.md`
- `./scripts/validate-plan-task-use-cases.sh -PlanPath ./docs/superpowers\plans\2026-06-21-m0-m1-workflow-contract-normalization-plan.md`
- `./scripts/validate.sh`
