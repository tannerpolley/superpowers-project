...
**Tech Stack:** Markdown, Codex plugin manifest JSON, PowerShell validation/sync scripts, GitHub issue template YAML.
...
- [ ] Sync/install defaults deploy to `C:\Users\Tanner\plugins\superpowers-project`.
- [ ] Sync removes this plugin's retired live copy at `C:\Users\Tanner\plugins\milestones`.
...
- [ ] **Step 2: Add clone/install instructions**
  Include the intended public repo name `codex-superpowers-project`, local clone instructions, `scripts\install.ps1`, validation, and live install locations.
...
- [ ] **Step 1: Change sync default**
  Set the default live plugin root to `$env:USERPROFILE\plugins\superpowers-project`.
