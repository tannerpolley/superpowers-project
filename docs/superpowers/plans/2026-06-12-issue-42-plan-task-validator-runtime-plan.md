...
**Goal:** Ship `scripts/validate-plan-task-use-cases.sh` as a tracked Superpowers Project plugin runtime script so planning and implementation gates can run from synced live and cached plugin surfaces.
...
**Tech Stack:** Bash sync scripts, plugin runtime surface hashing, live install drift checks, local scenario tests.
...
- `scripts/sync-live.sh` copies `scripts/validate-plan-task-use-cases.sh` into the live plugin `scripts` directory.
- `scripts/lib\plugin-cache.sh` copies `scripts/validate-plan-task-use-cases.sh` into refreshed local plugin cache roots.
- `scripts/lib\live-install.sh` detects missing or drifted live validator files.
- `scripts/get-agent-plugin-version.sh` includes the validator in the runtime contract hash.
- Focused tests prove live sync, cache refresh, live drift detection, and version freshness include the validator.
- Full validation and `scripts/sync-live.sh --validate` pass.
...
```bash
./scripts/validate-plan-task-use-cases.sh -PlanPath docs/superpowers/plans/2026-06-12-issue-42-plan-task-validator-runtime-plan.md
...
**Use Cases:**
- A live plugin install contains `scripts/validate-plan-task-use-cases.sh` after sync.
- A refreshed local plugin cache root contains `scripts/validate-plan-task-use-cases.sh`.
...
- Modify: `scripts/lib/plugin-cache.sh`
- Modify: `scripts/lib/live-install.sh`
- Modify: `scripts/get-agent-plugin-version.sh`

- [ ] **Step 1: Add the runtime validator path to sync-live**
  Add `$sourcePlanTaskUseCasesValidator = Join-Path $repoRoot "scripts/validate-plan-task-use-cases.sh"` near the other source runtime script variables.

- [ ] **Step 2: Require and copy the validator in sync-live**
  Validate that `$sourcePlanTaskUseCasesValidator` exists, then copy it to `Join-Path $livePluginScriptsRoot "validate-plan-task-use-cases.sh"` with `-Force`.

- [ ] **Step 3: Add the validator to cache runtime copying**
  In `scripts/lib/plugin-cache.sh`, add `$sourcePlanTaskUseCasesValidator`, include it in the required runtime surface list, and copy it to the target plugin `scripts` directory.

- [ ] **Step 4: Add live install drift detection**
  In `scripts/lib/live-install.sh`, compare the source validator and live validator with label `plugin plan task use-case validator`.

- [ ] **Step 5: Add version hash coverage**
  In `scripts/get-agent-plugin-version.sh`, include `scripts/validate-plan-task-use-cases.sh` in `Get-RuntimeContractEntries`.
...
- Modify: `scripts/test-agent-plugin-version.sh`

- [ ] **Step 1: Extend sync policy text assertions**
  Add `validate-plan-task-use-cases.sh` to the required sync-live policy text needles in `scripts/test-plugin-only-live-sync.sh`.

- [ ] **Step 2: Extend live fixture copying**
  Copy `scripts/validate-plan-task-use-cases.sh` into live and cached fixture plugin roots wherever the tests already copy the version checker and Auto Mode validator.
...
- [ ] **Step 4: Extend version runtime fixture copying**
  In `scripts/test-agent-plugin-version.sh`, copy the validator in `Copy-RuntimeSurface` so version tests model the real runtime surface.

- [ ] **Step 5: Add observed validator drift coverage**
  In `scripts/test-agent-plugin-version.sh`, alter only the observed plugin validator in a fixture and assert `-RequireCurrent` fails with an observed freshness reason.
...
```bash
./skills/create-issues/scripts/validate-issue-mirror.sh -IssueFile docs/superpowers/issues/42-ship-or-install-validate-plan-task-use-cases.md
./scripts/validate-plan-task-use-cases.sh -PlanPath docs/superpowers/plans/2026-06-12-issue-42-plan-task-validator-runtime-plan.md
```
