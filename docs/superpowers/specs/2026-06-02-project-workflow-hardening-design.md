...
- The `milestones-plugin` run that created issue #2, resolved it through `$project:resolve-issue`, merged PR #3 through `$project:merge-changes`, and used `$project:audit-project` for post-merge drift repair.
...
- `$project:write-plan` already has a native continuation gate after plan creation.
- `$project:resolve-issue` owns native goal-backed issue implementation and PR-ready handoff.
- `$project:merge-changes` owns merge approval, issue closure verification, branch/worktree cleanup, prune, cleanup hook, and clean repo proof.
- `$project:audit-project` is report-first and can repair drift only after native approval.
...
- Quick implementation path: add a guarded local-main path after `$project:write-plan` for small, low-risk changes.
- Closed issue mirror lifecycle: delete closed issue mirrors by default during `$project:merge-changes` closeout after GitHub issue closure is verified, unless the mirror is explicitly marked as historically retained.

## Problem Statement

The concept is working: native questions make the workflow explicit, `$project:resolve-issue` and `$project:merge-changes` split implementation from integration cleanly, and the local/GitHub issue backbone gives agents enough structure to work autonomously.
...
- `$project:write-plan` always routes toward issues or vanilla Superpowers execution, even when the work is a small quick change that should be applied directly with focused verification.
...
7. Add a guarded quick implementation route after `$project:write-plan`.
...
This should become a shared rule for all Superpowers Project skills, not only `$project:brainstorm-spec`:
...
- `$project:brainstorm-spec`: summarize the saved spec, then ask whether to continue to `$project:write-plan`, review first, or revise the spec.
- `$project:write-plan`: summarize the saved plan, then ask whether to continue to `$project:create-issues`, Quick Apply, subagent execution, inline execution, review first, or revise the plan.
- `$project:create-issues`: summarize created or updated issues, then ask whether to resolve the first ready issue, resolve a selected issue, review first, or stop.
- `$project:resolve-issue`: summarize PR-ready evidence, then ask whether to start `$project:merge-changes`, resolve another issue, review first, or stop.
- `$project:merge-changes`: summarize merge closeout evidence, then ask whether to run `$project:audit-project`, resolve another issue, review first, or stop.
- `$project:audit-project`: summarize findings and repair state, then ask whether to apply a repair plan, create a planning spec, run another audit, or stop.
...
During `$project:merge-changes` closeout:
...
`$project:audit-project` should report:
...
`$project:audit-project` should gain a scripted audit that emits structured findings grouped as blocking, repairable, informational, and healthy.
...
Add a fourth `$project:write-plan` continuation option:
...
Examples that should route to `$project:create-issues` and `$project:resolve-issue`:
...
- route non-small work back to `$project:create-issues`.

Update `$project:resolve-issue`:
...
Update `$project:brainstorm-spec`:
...
- Do not remove `$project:create-issues`, `$project:resolve-issue`, or `$project:merge-changes`.
...
- Should Quick Apply get its own bundled script, or should it stay as `$project:write-plan` documented behavior plus validation tests?
