...
$wrapperFile = Join-Path $repoRoot "skills\superpowers-project\SKILL.md"
...
try {
    ...
    try {
        Assert-Contains -Text $wrapper -Needle 'C:\Users\Tanner\.agents\skills\superpowers-project\SKILL.md' -Reason "wrapper missing deployed path"
        Assert-Contains -Text $wrapper -Needle 'Read the deployed user-level `SKILL.md` above.' -Reason "wrapper missing read instruction"
