...
try {
    ...
    Assert-Contains -Path "skills\companion-interface\SKILL.md" -Needle "Agent-Native visual-plan or visual-recap MDX" -Name "skill defines Agent-Native MDX surface"
    Assert-Contains -Path "skills\companion-interface\SKILL.md" -Needle "visual-recap" -Name "skill defines visual-recap surface"
    ...
    Assert-Contains -Path "skills\companion-interface\SKILL.md" -Needle "Docker preview" -Name "skill documents Docker preview boundary"
    Assert-Contains -Path "skills\companion-interface\agents\openai.yaml" -Needle "Agent-Native" -Name "metadata names Agent-Native"
    Assert-Contains -Path "skills\companion-interface\agents\openai.yaml" -Needle "visual-recap" -Name "metadata names visual-recap"
    ...
    Assert-Contains -Path ".codex-plugin\plugin.json" -Needle "Agent-Native visual-plan or visual-recap MDX" -Name "plugin prompt names MDX companion"
    Assert-Contains -Path ".codex-plugin\plugin.json" -Needle "visual-recap" -Name "plugin prompt names recap companion"
    Assert-Contains -Path "README.md" -Needle '$superpowers-project:companion-interface' -Name "README lists companion"
    Assert-Contains -Path "README.md" -Needle "Agent-Native visual-plan or visual-recap MDX" -Name "README names MDX companion"
    Assert-Contains -Path "README.md" -Needle "visual-recap" -Name "README names recap companion"
    Assert-Contains -Path "README.md" -Needle "Hosted comments, feedback tools, and full Agent-Native collaboration require hosted Plan tools or a proper local Plan app" -Name "README documents Docker preview boundary"
    ...
    Assert-Contains -Path "docs\superpowers\PROJECT_CONTEXT.md" -Needle "visual-recap artifacts" -Name "project context documents recap artifacts"
    Assert-Contains -Path "plans\agent-native-companion-replacement\plan.mdx" -Needle 'Use `visual-recap`' -Name "checked-in companion documents recap operating model"
    Assert-Contains -Path "docker-compose.agent-native-preview.yml" -Needle '${PLAN_KIND:-plan}' -Name "Docker preview kind is configurable"
    Assert-Contains -Path "docker-compose.agent-native-preview.yml" -Needle '${PLAN_DIR:-/repo/plans/agent-native-companion-replacement}' -Name "Docker preview plan dir is configurable"
