...
try {
    ...
    Write-BacklogFixture -Path $validBacklog -Rows @(
        "| 71 | resolve-issue | docs/superpowers/issues/71-golden-path-workflow-fixtures.md | P2 | ready | pwsh.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\test-workflow-examples.ps1 | Add golden-path workflow fixtures. |",
        "| 72 | resolve-issue | docs/superpowers/issues/72-live-sync-tracker-align-validation.md | P2 | blocked | pwsh.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\validate.ps1 | Blocked by workflow examples. |"
    )
    ...
    Write-BacklogFixture -Path $missingProof -Rows @(
        "| 71 | resolve-issue | docs/superpowers/issues/71-golden-path-workflow-fixtures.md | P2 | ready |  | missing proof target |"
    )
    ...
    Write-BacklogFixture -Path $unsupportedRoute -Rows @(
        "| 99 | unknown-route | docs/superpowers/issues/71-golden-path-workflow-fixtures.md | P2 | ready | pwsh.exe -File fixture.ps1 | bad route |"
    )
    ...
    Add-Check -Name "selector chooses active backlog item" -Ok ($selection.exit_code -eq 0 -and $selection.json.selected_candidate_id -eq "71") -Reason $selection.raw
